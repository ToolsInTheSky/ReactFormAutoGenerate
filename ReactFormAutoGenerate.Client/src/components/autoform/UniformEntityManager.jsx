...
import { registerSchemaHeadersInterceptor } from "./schemaHeaders";
 ...
 */
export const UniformEntityManager = ({
	protocol = "rest",
	resource,
	entityName,
	schemaUrl,
	...
	// Register schema headers interceptor once on Refine's axios instance
	React.useEffect(() => {
		registerSchemaHeadersInterceptor(axiosInstance);
	}, []);
				...
				if (data.properties) {
					Object.entries(data.properties).forEach(async ([key, prop]) => {
						...
						if (relRes) {
							let items = [];
							if (isRest) {
								const res = await fetch(`/api/${relRes}/all`);
								const list = await res.json();
								items = (Array.isArray(list) ? list : list.data || []).map(
									(item) => ({
								...
								items = (res.data?.data?.[qName]?.items || []).map((i) => ({
									label: i.name || i.id,
									value: i.id,
								}));
							}
	...
	useEffect(() => {
		if (isRest || schema) fetchData();
	}, [isRest, schema, fetchData]);

	const entities = useMemo(
		() => (isRest ? listData?.data || records : records),
		[isRest, listData, records],
	);
	...
	// selectedRecord is set directly from the grid row click (e.dataItem),
	// so it is always in sync regardless of list loading state.

	if (
		loading ||
		!schema ||
		(isRest && isListLoadingRest) ||
		(!isRest && isListLoadingGql)
	)
		return (
	...
	if (fetchError)
		return <div style={{ color: "red", padding: "20px" }}>{fetchError}</div>;
	...
	return (
		<div style={{ width: "100%" }}>
			...
			<Grid
				...
				schema={schema}
				lookups={lookupsMap}
				isLoading={loading || (isRest ? isListLoadingRest : isListLoadingGql)}
				onRefresh={fetchData}
				onAdd={() => {
					setSelectedId(null);
