...
const RjsfEntityManager = ({
	...
	const records = useMemo(() => {
		...
		if (isRest) {
			return Array.isArray(listResponse)
				? listResponse
				: listResponse.data || [];
		} else {
	...
	const lookupQueriesResults = useQueries({
		queries: autoRelations.map((rel) => ({
			queryKey: isRest
				? ["lookup", rel.resource, rel.field]
				: ["gql-lookup", rel.resource],
			queryFn: async () => {
				if (isRest) {
	...
	if (schemaError)
		return (
			<div style={{ color: "red" }}>Schema Error: {schemaError.message}</div>
		);
